 ...
 *   bun run src/index.ts [folder-path] [--parallel N]
 *
 * Examples:
 *   bun run src/index.ts
 *   bun run src/index.ts --parallel 5
 *   bun run src/index.ts -p 3
 *   bun run src/index.ts /path/to/photos --parallel 4
 *   bun run src/index.ts -f=/path/to/photos -p=2
 */
...
function parseCommandLineArgs() {
  ...
  if (showHelp) {
    console.log(`
Usage: bun run src/index.ts <folder-path> [options]
...
Examples:
  ...
  bun run src/index.ts /path/to/photos -p 3
  bun run src/index.ts -f=/path/to/photos --parallel 4
  bun run src/index.ts -f=/path/to/photos -p=2
`);
    process.exit(0);
  }

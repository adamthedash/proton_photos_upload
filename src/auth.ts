 ...
 */
function uint8ArrayToBigIntBE(arr: Uint8Array): bigint {
  let result = 0n;
...
function uint8ArrayToBigIntLE(arr: Uint8Array): bigint {
  return uint8ArrayToBigIntBE(arr.slice().reverse())
}
...
async function formatHash(
  ...
  const merged = mergeUint8Arrays([hashBytes, modulus]);
  return expandHash(merged);
}
...
async function generateProofs({
  ...
  // modulus == N
  const modulus = uint8ArrayToBigIntLE(modulusArray);
  ...
  // expandHash == H
  const m = mergeUint8Arrays([generatorArray, modulusArray]);
  const multiplierHash = await expandHash(m);
  // multiplier == k
  const multiplier = uint8ArrayToBigIntLE(multiplierHash);


  const serverEphemeral = uint8ArrayToBigIntLE(serverEphemeralArray);
  const hashedPassword = uint8ArrayToBigIntLE(hashedPasswordArray);
  ...
  for (let i = 0; i < 1000; i++) {
    ...
    // clientSecret == a
    clientSecret = uint8ArrayToBigIntLE(randomBytes);
    // clientEphemeral == a_pub
    ...
    // scramblingParam == u
    scramblingParam = uint8ArrayToBigIntLE(clientServerHash);
...
export function createSrpModule(): SRPModuleInterface {
  return {
    ...
    async getSrpVerifier(password: string): Promise<SRPVerifier> {
      ...
      // Generate verifier
      const generator = 2n;
      const modulusBigInt = uint8ArrayToBigIntLE(modulus);
      const hashedPasswordBigInt = uint8ArrayToBigIntLE(hashedPassword);
